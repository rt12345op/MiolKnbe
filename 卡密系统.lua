...
local function verifyKey(keyToVerify, isAutoLogin)
    ...
    local allowedKey = USER_ID_MAP[playerId]
    ...
    local function kickPlayerFunction(errorMessage)
        ...
        KeyInput.Text = ""
        Main.Rotation = 0
        local shakeInfo = TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut)
        local shakeTween1 = TweenService:Create(Main, shakeInfo, {Rotation = 5})
        local shakeTween2 = TweenService:Create(Main, shakeInfo, {Rotation = -5})
        local shakeTween3 = TweenService:Create(Main, shakeInfo, {Rotation = 3})
        local shakeTween4 = TweenService:Create(Main, shakeInfo, {Rotation = -3})
        local shakeTween5 = TweenService:Create(Main, shakeInfo, {Rotation = 0})
        shakeTween1:Play()
        shakeTween1.Completed:Wait()
        shakeTween2:Play()
        shakeTween2.Completed:Wait()
        shakeTween3:Play()
        shakeTween3.Completed:Wait()
        shakeTween4:Play()
        shakeTween4.Completed:Wait()
        shakeTween5:Play()
        wait(2)
        SubmitBtn.Text = "Verify Key"
        SubmitGradient.Color = ColorSequence.new{
...
Learn more at https://moe-chain.xyz/]==]
            ...
            if not allowedKey then
                return kickPlayer("Unauthorized User")
            end
            
            if getfenv().ADittoKey ~= allowedKey then
                return kickPlayer("Invalid Key")
            end
            
            ADitto_UserGroup = {"Default"}
            ADitto_Premium = true
            ADitto_KeyType = "permanent"
